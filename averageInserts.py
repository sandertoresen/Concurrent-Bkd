...
for i in program_numbers:
    ...
    for j in range(10):
        ...
        output = subprocess.check_output(
            [program_name, program_arg]).decode("utf-8")
        elapsed_time_str = re.search(
            r"Time elapsed (\d+\.\d+)s", output).group(1)
    ...
    print(
        f"64part no delay: program {i}: average run time {avg_elapsed_time:.2f} seconds!")

print("\nAll average times:")
for i, avg_time in enumerate(avg_times):
    print(
        f"program {program_numbers[i]} average run time {avg_time:.2f} seconds")
